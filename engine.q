...
.eng.match_mbo:{
	...
	so:first .eng.laq;
	if[bo[`qty] < so[`qty];
	t:(bo[`oid]; so[`oid]; .z.N; bo[`sym]; so[`price]; bo[`qty]);
	...
	if[bo[`qty] > so[`qty];
	t:(bo[`oid]; so[`oid]; .z.N; bo[`sym]; so[`price]; so[`qty]);
	(neg .eng.ex_h) (".ex.recv_trade"; t);
	bo[`qty]-:so[`qty];
	so[`qty]:0;
...
.eng.match_mao:{
	...
	bo:first .eng.lbq;
	if[so[`qty] < bo[`qty];
	t:(bo[`oid]; so[`oid]; .z.N; bo[`sym]; bo[`price]; so[`qty]);
	(neg .eng.ex_h) (".ex.recv_trade"; t);
	bo[`qty]-:so[`qty];
	so[`qty]:0;
	...
	if[so[`qty] > bo[`qty];
	t:(bo[`oid]; so[`oid]; .z.N; bo[`sym]; bo[`price]; bo[`qty]);
	(neg .eng.ex_h) (".ex.recv_trade"; t);
	so[`qty]-:bo[`qty];
	bo[`qty]:0;
...
.eng.order_book:{
	0N!"Ask Side:";
	show 5#`price xasc (select sum qty by price from .eng.laq);
	0N!"Bid Side:";
	show 5#`price xdesc (select sum qty by price from .eng.lbq);}
